...
**Standard.** Deployment MUST use the narrowest supported reload or restart that applies the change. A full Home Assistant Core restart requires a documented reason, impact check, and post-restart verification when a domain or integration reload would suffice.
